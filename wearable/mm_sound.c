...
static char *volume_type_str[VOLUME_TYPE_MAX] = { "SYSTEM", "NOTIFICATION", "ALARM", "RINGTONE", "MEDIA", "CALL", "VOIP", "SVOICE", "FIXED", "JAVA" };
...
static int __validate_volume(volume_type_t type, int value)
{
	...
	case VOLUME_TYPE_RINGTONE:
		...
		break;
	case VOLUME_TYPE_SVOICE:
		if (value >= AVSYS_AUDIO_VOLUME_MAX_SVOICE) {
			return -1;
		}
		break;
...
int mm_sound_mute_all(int muteall)
{
	...
	ret = avsys_audio_set_mute_all(muteall);
	if (ret != MM_ERROR_NONE) {
		debug_error("Can not set muteall to shared memory 0x%x\n", ret);
	}
...
int mm_sound_set_call_mute(volume_type_t type, int mute)
{
	...
	ret = avsys_audio_set_call_mute(type, mute);
	if (ret != MM_ERROR_NONE) {
		debug_error("set call mute failed: 0x%x\n", ret);
	}
	return ret;
}
...
int mm_sound_get_call_mute(volume_type_t type, int *mute)
{
	...
	ret = avsys_audio_get_call_mute(type, mute);
	if (ret != MM_ERROR_NONE) {
		debug_error("get call mute failed: 0x%x\n", ret);
	}
	return ret;
}
...
int mm_sound_set_factory_loopback_test(int loopback)
{
	...
	ret = avsys_audio_set_factory_loopback_test(loopback);
	if (ret != MM_ERROR_NONE) {
		debug_error("set loopback failed: 0x%x\n", ret);
	}
	return ret;
}
...
int mm_sound_get_factory_loopback_test(int *loopback)
{
	...
	ret = avsys_audio_get_factory_loopback_test (loopback);
	if (ret != MM_ERROR_NONE) {
		debug_error("get loopback failed: 0x%x\n", ret);
	}
	return ret;
}
...
int mm_sound_volume_get_current_playing_type(volume_type_t *type)
{
	...
	debug_msg("returned type = (%d)%15s, avsys result = 0x%x, ret = 0x%x", *type, __get_volume_str(*type), result, ret);
...
EXPORT_API
int mm_sound_set_active_route_auto()
{
	int ret = MM_ERROR_NONE;
