...
/* PopSync status value */
static void _popsync_status_changed_cb(keynode_t* node, void* data)
{
	int popsync_status = 0;

	/* Get actual vconf value */
	vconf_get_bool(VCONFKEY_POPSYNC_ACTIVATED_KEY , &popsync_status);
	debug_msg ("[%s] changed callback called, status=[%d]\n", vconf_keynode_get_name(node), popsync_status);

	MMSoundMgrSessionSetDeviceAvailable (DEVICE_MIRRORING, popsync_status, 0, NULL);
}

static int _register_popsync_status(void)
{
	/* set callback for vconf key change */
	int ret = vconf_notify_key_changed(VCONFKEY_POPSYNC_ACTIVATED_KEY , _popsync_status_changed_cb, NULL);
	debug_msg ("vconf [%s] set ret = [%d]\n", VCONFKEY_POPSYNC_ACTIVATED_KEY , ret);
	return ret;
}
...
int MMSoundMgrWfdInit(void)
{
	...
	if (_register_popsync_status() != 0) {
		debug_error ("Registering POPSYNC status failed\n");
		return MM_ERROR_SOUND_INTERNAL;
	}
